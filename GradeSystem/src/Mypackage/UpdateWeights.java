 ...
 * 
 * Time estimate: O(1)
 * Example: count = count_100(W, w, 0, weights, new_weights);
 * 
...
public class UpdateWeights {
	...
	public static int [] weights(int []weights) {
		...
		while(count != 100) {
			...
			count = count_100(W, w, 0, weights, new_weights);
		}
		/* 2. Ask user to make sure the change is correct. */
	...
	static int count_100(Scanner W, String w, int count, int[]weights, int []new_weights) {
		System.out.printf("\tlab1\t%d%%\n\tlab2\t%d%%\n\tlab3\t%d%%\n\tmid-term\t%d%%\n\tfinal-exam\t%d%%\n",weights[0],weights[1],weights[2],weights[3],weights[4]);
		w = W.nextLine();
		String [] neww = w.split(" ");
		for(int i=0;i<5;i++) {
